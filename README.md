...
- **🔄 流式响应**: 实时流式输出技能执行结果
- **🌐 多语言支持**: 中文、日语（半角外来语）、英语三语言界面切换
- **🎨 紫色渐变设计**: 现代的紫色梯度主题和优化的 UI 布局
...
- **可折叠组件**: 推理过程和完整结果可折叠
- **实时状态**: 显示正在调用的技能、当前使用的模型和已加载技能数
- **Pipeline 可视化**: 清晰显示串行/并行执行流程
- **动态扩展提示**: 当系统自动追加技能时显示 "✨" 标记
- **紫色梯度主题**: 从蓝紫到洋紫的现代渐变设计
- **多语言界面**: 侧边栏语言切换器，实时切换中文/日语/英语
- **优化的聊天气泡**: 紧凑的布局，用户消息为紫色右对齐，助手消息为白色左对齐
- **响应式设计**: 适配不同屏幕尺寸
- **侧边栏管理**: 技能列表和文档管理，带语言名称本地化
